...
class TBHalloween {
  ...
  /**
   * Constructs a new Halloween service.
   *
  ...
  public function __construct(PathMatcherInterface $path_matcher, AliasManagerInterface $alias_manager, ConfigFactoryInterface $config_factory, AdminContext $admin_context) {
    ...
    $this->config = $config_factory->get('tb_halloween.settings');
    $this->adminContext = $admin_context;
  }

    /**
     * Checks whether a give path matches the halloween path rules.
     ...
     */
  public function isTBHalloweenEnabledPath(Url $url) {

    if ($url->isExternal()) {
      return FALSE;
    }

    // If we don't want to enable tb_halloween on admin pages.
    ...
    $patterns = strtolower($this->config->get('tb_halloween_patterns'));
